...
var mergeAlternately = function(word1, word2) {
    let merged = ''; //Initialize an empty string
    let i = 0, j = 0;
  
    //Loop while there are characters in both strings
    while(i < word1.length && j < word2.length) {
      merged += word1[i]; // add from word1
      merged += word2[j]; // Add from word2
      i++; // Increment word1 index
      j++; // Increment word2 index
    }
  
    //Append remaining characters from the longer string
    merged += word1.slice(i); //If any left in word1
    merged += word2.slice(j); // If any left in word2
